...
> 本文件保留第一阶段 `$V,...#` ASCII 联调协议，主要供 MaixCAM 假数据和 STM32F407 接收端兼容使用。MSPM0G3507 云台链路已切换为二进制协议，以 `docs/maixcam_protocol.md` 为当前基准；视觉、STM32 与系统集成负责人修改任一协议前都必须先更新对应文档并通知全队。
...
- 视觉负责人：发送端字段顺序、缩放单位和发送周期不得自行更改。
- STM32 与执行机构负责人：解析代码必须严格按本文件实现，不得为临时方便另定义格式。
- 系统集成负责人：MSPM0 解析逻辑与 STM32 保持一致，并负责接口变更审查。
