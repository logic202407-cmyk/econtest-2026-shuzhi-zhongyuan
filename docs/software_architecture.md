...
MaixCAM Pro --UART1--> vision/maixcam_protocol
                        ...
                        gimbal/gimbal_control
                               |
                               v
                      motor/x42s_rs485 --UART2 + RS485--> X42S
                               ^
                               |
...
| 电机协议层 | `application/motor/x42s_rs485/x42s_rs485.*` | 封装 X 固件自由协议，不绑定具体 UART 驱动 |
...
每轮最多处理 `APP_UART_POLL_BUDGET` 个 UART1 字节和同等数量的 UART2 字节，避免高频串口数据永久占用主循环。UART ISR/DMA 只写入接收 FIFO；平台的 `App_VisionReadByte()`、`App_MotorReadByte()` 从 FIFO 取数据。
...
- UART2 回复：仅更新 X42S 最近位置/速度缓存，不阻塞控制循环等待回复。
