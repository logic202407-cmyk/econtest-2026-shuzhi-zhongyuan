...
|LED|PB22|板载 LED|
|KEY|PB21|低电平按下|
|RS485_DE|PB17|手动方向 RS485 模块预留；当前自动方向模块不连接|

当前选用的 TTL-RS485 小模块为自动方向型：PB15 接模块 TXD、PB16 接模块 RXD、模块供电为 3.3 V，PB17 不连接该模块。若后续改用普通半双工 RS485 芯片，建议把 DE 与 `/RE` 短接后连接到 PB17：
